...
export const circomLanguage = LRLanguage.define({
	parser: parser.configure({
		props: [
			indentNodeProp.add({
				"FunctionBody TemplateBody IfBody ElseBody ForLoopBody WhileLoopBody": delimitedIndent({ closing: "}" }),
			}),
			foldNodeProp.add({
				"FunctionBody TemplateBody IfBody ElseBody ForLoopBody WhileLoopBody": foldInside,
				BlockComment(tree) {
